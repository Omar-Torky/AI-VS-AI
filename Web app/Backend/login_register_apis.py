...
def send_verification_email(email, verification_code):
    ...
    msg = EmailMessage()
    msg['Subject'] = 'Your AI VS AI OTP Code'
...
def logout_user():
    if 'user_id' or 'user_email' in session:
...
def search_user_by_email():
    ...
    try:
        cursor = conn.cursor()
        cursor.execute("SELECT name, email, verification_code, verification_code_expiry FROM [User] WHERE email = ?", (email,))
        user = cursor.fetchone()

        if user:
            name, email, verification_code, verification_code_expiry = user
            expiry_str = verification_code_expiry.isoformat() if verification_code_expiry else None
            return jsonify({
                "message": "User found.",
                "data": {
                    "name": name,
                    ...
                    "verification_code_expiry": expiry_str
                }
            }), 200
        else:
            return jsonify({"error": "User not found."}), 404
